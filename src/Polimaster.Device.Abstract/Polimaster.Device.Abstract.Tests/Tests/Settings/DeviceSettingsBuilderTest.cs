...
public class DeviceSettingsBuilderTest : Mocks {
    ...
    public void ShouldBuildImplementation() {
        var setting = _builder.WithReader(_readerMock.Object)
            .WithWriter(_writerMock.Object).WithImplementation<MyDeviceSetting, MyParam>().Build<MyParam>();
        
        Assert.IsType<MyDeviceSetting>(setting);
    }

    [Fact]
    public void ShouldBuildProxy() {
        var setting = _builder.WithReader(_readerMock.Object)
            .WithWriter(_writerMock.Object).BuildWithProxy<MyParamSettingProxy, string, MyParam>();
        
        Assert.IsType<MyParamSettingProxy>(setting);
    }
}